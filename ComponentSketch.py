...
def rounded_rect(canvas: tk.Canvas, x: int, y: int, width: int, height: int, radius: int, thickness: int, **kwargs) -> None:
    """
    Draws a rounded rectangle on a given canvas.
    Parameters:
    canvas (tk.Canvas): The canvas on which to draw the rounded rectangle.
...
def drawChar(canvas, xD, yD, scale=1, width=-1, direction=HORIZONTAL, **kwargs):
    ...
    if angle != 0:
        ...
        canvas.create_text(xD, yD, text=text, font=firaCodeFont, fill=color, anchor=anchor, tags=tags)
...
def drawredRail(canvas, xD, yD, scale=1, width=-1, direction=HORIZONTAL, **kwargs):
    ...
    (x, y) = drawRail(canvas, xD, yD - inter_space // 2, scale, width, direction, color="red")
...
def drawBoard(canvas, xD=0, yD=0, scale=1, width=-1, direction=VERTICAL, **kwargs):
    ...
    rounded_rect(canvas, xD, yD, dimLine, dimColumn, radius, outline=color, fill=color, thickness=thickness)
    for sepA in sepAlim:
        canvas.create_line(
    ...
    # canvas.create_line(xD , yD+inter_space*11+inter_space//3, xD + dimLine, yD + inter_space*11+inter_space//3, fill="#c0c0c0", width=(3*inter_space)//5)
    # if direction == HORIZONTAL:
    #     xD += dimLine
    # else: yD += dimColumn
...
def drawPin(canvas, xD, yD, scale=1, width=-1, direction=HORIZONTAL, orientation=1, **kwargs):
    ...
    # canvas.create_line(xD - inter_space//2, yD + 8*inter_space//15, xD - inter_space//2, yD + 13*inter_space//15, fill="#ffffff", width=1)
    # canvas.create_line(xD - inter_space//2, yD + 13*inter_space//15, xD , yD + 13*inter_space//15, fill="#ffffff", width=1)
    canvas.create_line(
        xD - 3 * inter_space // 15,
        yD + orientation * 5 * inter_space // 15,
        xD - 3 * inter_space // 15,
...
def drawOR(canvas, xD, yD, scale=1, width=-1, direction=HORIZONTAL, orientation=1, **kwargs):
    ...
    # canvas.create_line(xD + 1*inter_space//2, yD, xD + 1*inter_space//2, yD + 2*inter_space//3, fill="#ffffff", width=1)
    canvas.create_line(
        xD + 9 * inter_space // 15,
        yD + orientation * 3.5 * inter_space // 15,
        xD + 12 * inter_space // 15,
...
def drawSwitch(
    canvas, x1, y1, fillSupport="#fffffe", fillSwitch="#ff0000", outSwitch="#000000", posSwitch=LEFT, tag=None, numBtn=1
):
    canvas.create_arc(x1, y1, x1 + 20, y1 + 20, start=90, extent=180, fill=fillSupport, outline=fillSupport, tags=tag)
    canvas.create_arc(
...
def onDragMenu(event, canvas, tag):
    ...
    canvas.move(tag, event.x - drag_mouse_x, event.y - drag_mouse_y)
...
def drawMenu(canvas, xMenu, yMenu, thickness, label, tag, id):
    ...
    if params:
        ...
        rounded_rect(canvas, xMenu, yMenu, 128, 128, 10, outline=outMenu, fill=fillMenu, thickness=thickness, tags=tag)
        ...
        canvas.tag_bind(
            "btn1_switch_" + tag, "<Button-1>", lambda event: onSwitch(event, canvas, "btn1_switch_" + tag, id, 1)
        )
        ...
        canvas.tag_bind(
            "btn2_switch_" + tag, "<Button-1>", lambda event: onSwitch(event, canvas, "btn2_switch_" + tag, id, 2)
        )
        ...
        canvas.tag_bind(
            "btn3_switch_" + tag, "<Button-1>", lambda event: onSwitch(event, canvas, "btn3_switch_" + tag, id, 3)
        )
...
def drawChip(canvas, xD, yD, scale=1, width=-1, direction=HORIZONTAL, **kwargs):
    ...
    if not tags:
        ...
        zone = rounded_rect(
            canvas, xD, yD, dimLine, dimColumn, 5, outline="#343434", fill="#343434", thickness=thickness, tags=tagBase
        )
        ...
        if dim["internalFunc"] != None:
            dim["internalFunc"](canvas, xD, yD, scale=scale, tags=tagBase, **kwargs)

        rounded_rect(
            canvas, xD, yD, dimLine, dimColumn, 5, outline="#343434", fill="#343434", thickness=thickness, tags=tagCapot
        )
        canvas.create_line(
        ...
        canvas.tag_bind(
            tagSouris, "<Button-2>", lambda event: onMenu(event, canvas, tagMenu, "componentMenu", tagSouris)
        )
    else:
        X, Y = params["XY"]
        dX = xD - X
