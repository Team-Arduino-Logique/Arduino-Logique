...
from dataComponent import circuitTest


def zoom(p_canvas: tk.Canvas, p_scale: float, p_board: Breadboard, p_board_x: int, p_board_y: int) -> None:
    """
    ...
    p_board.circuit(p_board_x, p_board_y, scale=int(p_scale) / 10.0, model=circuitTest)
...
if __name__ == "__main__":
    ...
    zoom(canvas, 10.0, board, 50, 10)

    h_slider = tk.Scale(
        win, from_=10, to=30, orient="horizontal", command=lambda scale: zoom(canvas, float(scale), board, 50, 10)
    )
